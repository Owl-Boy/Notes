...
>[!important] Setup
>There are two players, a *spoiler* and a *duplicator*. And the board consists of two structure $\mathfrak A$ and $\mathfrak B$. The goal of the *spoiler* is to show that the two structures are different, and the goal of the *duplicator* is to show that the two structures are the same.
>
...
We define $\vec{c^ \mathfrak A}=c^{ \mathfrak A}_{1}\dots c^ \mathfrak A_{k}$ and $\vec{c^\mathfrak B}=c^\mathfrak B_{1}\dots c^\mathfrak B_{k}$ is the interpretation of the constants in the two structures $\mathfrak B$ and $\mathfrak A$.

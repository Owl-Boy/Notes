...
$$
Given any finite subset of the theory, it is clear that it will have a model for it (Just a line graph would work), so by [[Logical Compactness]] we can see that the theory will be consistent, which will imply there will be a model for but, but that means $c_{1}$ and $c_{2}$ are consistent but there is no path of any length between them.
